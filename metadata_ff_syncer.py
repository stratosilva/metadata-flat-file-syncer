...
def instances():
    if exists('instances.conf'):
        ...
        for line in instance_file:
            ...
            if stripped_line == 'http://localhost:8080':
...
def import_metadata(metadata_type_selection: str):
    ...
    def convert_json_if_needed(element, key):
        ...
        # Check if it's a string that looks like a list or dictionary
        if isinstance(value, str) and value.strip().startswith(("[", "{")):
            try:
                # 1. Try standard JSON first (Fastest/Best practice)
                return json.loads(value)
            except (json.JSONDecodeError, ValueError):
                try:
                    # 2. Try Python literal evaluation (Handles single quotes & False/True/None)
                    return ast.literal_eval(value)
                except (SyntaxError, ValueError):
                    # 3. If both fail, it's just a string that happens to have brackets
                    return value
        return value
        ...
        if metadata_type in sheets_to_process:
            ...
            def convert_nulls(obj):
                ...
                def _convert(o):
                    ...
                    if isinstance(o, dict):
                        if set(o.keys()) == {"id"} and o["id"] == "null":
                            changed = True
                            return None
                ...
                cleaned_obj = _convert(obj)
                return cleaned_obj, changed

            null_value_present = False
            json_payload, null_value_present = convert_nulls(json_payload)
            ...
            if metadata_type_selection not in metadata_types_supported:
                ...
                if not gen_json:
                    ...
                    # Do not update the server if we are just generating the json
                    # For nulls to delete fields, the mode must be "REPLACE"
                    if null_value_present:
                        result = post_to_server(api_source, json_payload, mergeMode='REPLACE')
                    else:
